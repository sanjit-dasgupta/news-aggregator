...
{
	/** @var int */
	public static $cacheExpire = '1 minute';
	...
	private static function fromRss(SimpleXMLElement $xml)
	{
		...
		foreach ($xml->channel->item as $item){
			...
			} elseif (isset($item->{'a10:updated'})) {
				$item->timestamp = strtotime($item->{'a10:updated'});
			}
			
			// load image
			if(isset($item->image)){
				if(count($item->image->attributes()) > 0 && isset($item->image->attributes()->{'src'})){
					$item->img = $item->image->attributes()->{'src'};
				}else{
					$item->img = $item->image;
				}
			}else if(isset($item->fullimage)){
				$item->img = $item->fullimage;
			}
			if(!isset($item->img)){
				$item->img = "Not Found";
			}
